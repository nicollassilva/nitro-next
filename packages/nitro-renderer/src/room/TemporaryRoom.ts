import { GetObjectDataForFlags, IGetImageListener, IObjectData, IRoomGeometry, IRoomObjectController, ITemporaryRoom, IVector3D, ObjectDataFlagsEnum, RoomGeometryScaleType, RoomObjectCategoryEnum, RoomObjectUserTypeName, RoomObjectVariableEnum, Vector3d } from "@nitrodevco/nitro-api";
...
export class TemporaryRoom extends Room implements ITemporaryRoom {
    ...
    public async getGenericRoomObjectImage(
        ...
        const loaded = !!GetRoomContentLoader().getCollection(type);
        ...
        if (!loaded && listener) {
            ...
            this.removeRoomObject(objectId, objectCategory);
    ...
    public initalizeTemporaryObjectsByType(type: string): void {
        ...
        const objectCategory = GetRoomContentLoader().getCategoryForType(type);
        const objectManager = this.getRoomObjectManager(objectCategory);
