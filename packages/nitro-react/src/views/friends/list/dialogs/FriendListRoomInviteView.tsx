...
import { useFriendsContext, useIsWindowVisible, useSystemActions, useTranslation, useWebSocketContext } from "#base/context";
    ...
    return (
        <Frame variant="0" id="friendlist-room-invite" className="w-52.75 h-43.75" caption={t('friendlist.invite.title')} onClose={() => toggleWindow('friendlist_invite')}>
            <Border className="h-29 px-2.25 py-1">
                <div className="block overflow-hidden whitespace-nowrap text-[0.7rem] mb-0.5">{t('friendlist.invite.summary', '', { 'count': selectedFriendsIds.length.toString() })}</div>
